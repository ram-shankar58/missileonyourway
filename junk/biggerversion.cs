...
public class AdvancedPlayer : DrawableGameComponent
{
    ...
    public AdvancedPlayer(Game game, Texture2D texture, int x, int y) : base(game)
    {
        ...
        missileDelay = TimeSpan.FromMilliseconds(500);
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (keyboardState.IsKeyDown(Keys.Space))
        {
            ...
            if (currentTime - previousMissileTime > missileDelay)
            {
                FireMissile();
                previousMissileTime = currentTime;
            }
        }
    ...
    {
        Missile missile = new Missile(Game, texture, position.X + texture.Width / 2, position.Y - 10);
...
public class Missile : DrawableGameComponent
{
    ...
    public Missile(Game game, Texture2D texture, int x, int y) : base(game)
    {
        this.texture = texture;
        position = new Vector2(x, y);
        rectangle = new Rectangle(x, y, texture.Width, texture.Height);
